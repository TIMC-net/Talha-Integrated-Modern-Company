...
type Props = {
  /**
   * Web3Forms access key — intended to be used from the browser (their free tier
   * blocks server-side / proxy submissions). Safe to expose per Web3Forms docs.
   */
  web3formsAccessKey: string;
};

export default function ContactPageClient({ web3formsAccessKey }: Props) {
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (!/^\d{7,15}$/.test(phoneDigits)) {
      setError("Please enter a valid phone number (digits only, 7–15 numbers).");
      return;
    }
    ...
    try {
      // Bots filled the honeypot — fake success
      if (honeypot) {
        setSent(true);
        return;
      }
      ...
      if (!web3formsAccessKey) {
        setError(
          "Contact form is not configured. Add WEB3FORMS_ACCESS_KEY in Vercel environment variables and redeploy.",
        );
        return;
      }

      // Client-side submit required: Web3Forms free tier blocks server/proxy IPs
      // (Vercel Functions count as server-side). Access key is designed to be public.
      const res = await fetch("https://api.web3forms.com/submit", {
        ...
        body: JSON.stringify({
          ...
          subject: `TIMC Project Enquiry — ${service}`,
      ...
      if (!res.ok || payload?.success === false) {
        setError(
          payload?.message
            ? `${payload.message} If this continues, email ${company.email}.`
                ...
                <AnimatePresence mode="wait">
                  {sent ? (
                    ...
                    >
                      ...
                      />
                      {/* Web3Forms botcheck — must stay unchecked */}
                      <input
                        type="checkbox"
                        name="botcheck"
                        className="hidden"
