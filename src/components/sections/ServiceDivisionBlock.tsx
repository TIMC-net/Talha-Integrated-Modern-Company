...
function EngineeringPanel({
  ...
  return (
    ...
    >
      {/* Accent edge */}
      <span className="absolute top-0 bottom-0 left-0 z-20 w-[3px] origin-top scale-y-0 bg-accent transition-transform duration-700 ease-out group-hover:scale-y-100" />
...
export default function ServiceDivisionBlock({
  ...
  return (
    <section
      id={service.slug}
      data-dark-surface
      className={`relative scroll-mt-24 overflow-hidden border-t border-white/10 py-12 md:scroll-mt-28 md:py-16 lg:py-24 ${
            ...
            <Reveal delay={0.15} className="mt-9 flex w-full flex-col gap-3 sm:flex-row sm:flex-wrap">
              ...
              {isIntegrated && onViewFleet && (
                <Button
                  type="button"
                  variant="outline"
                  className="w-full border-white/20 text-white hover:border-accent hover:bg-accent hover:text-navy-950 sm:w-auto"
                  onClick={onViewFleet}
